...
naiveBayes <- setRefClass("naiveBayes",
       ...
       methods = list(
                    ...
                    fit = function(X, y)
                    {
                      tidy_text <- unnest_tokens(train, 'splitted', 'Message', token="words") %>%
             filter(!splitted %in% splitted_stop_words)
                      tidy_text <- tidy_text %>% count(tolower(splitted), sort=TRUE, Category) %>% pivot_wider(names_from = Category, values_from = n)
                      ...
                      tidy_text['ham_prob'] = tidy_text['ham']/ham
                      tidy_text['spam_prob'] = tidy_text['spam']/spam
                   ...
                   # parse = function()
                    #{
                      #tidy_test <- unnest_tokens(test, 'splitted', 'Message', token="words") %>%
            # filter(!splitted %in% splitted_stop_words)
                    ...
                    predict = function(message)
                    {
                      ...
                      message <- message[[1]]
                      for (val in message){
                        #idx <- which(tidy_text$splitted == val)
                        print(val)
                        #idx <- tidy_text$ham_prob[match(val, tidy_text$splitted)]
                        #print(idx)
                        #prob <- tidy_text[idx, 4]
                        #print(prob)
                      }
